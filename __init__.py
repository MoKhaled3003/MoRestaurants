...
CLIENT_ID = json.loads(
    open(os.path.join(app.root_path, 'google_client_secrets.json'), 'r').read())['web']['client_id']
...
def gconnect():
    ...
    try:
        ...
        oauth_flow = flow_from_clientsecrets(
            os.path.join(app.root_path, 'google_client_secrets.json'), scope='')
...
def showMenuByCategory(restaurant_id, category):
    ...
    restaurant = session.query(Restaurant).filter_by(id=restaurant_id).one()
    menuItems = session.query(MenuItem).filter_by(
        restaurant_id=restaurant_id, category=category).all()
